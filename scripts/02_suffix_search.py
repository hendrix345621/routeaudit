"""Stage 02 — RouteAudit universal-suffix search (needs 01 experts).

Optimizes one adversarial suffix that suppresses safety-expert routing, promotes
harmful experts, and blocks early refusal (the RouteHijack paper, arXiv 2605.02946).
Writes the suffix + per-prompt completion transcripts + TESR/THPR routing-shift diagnostics.
"""
...
from routeaudit.pipeline import attack_run
...
def main():
    ...
    p.add_argument("--universal-out", default="artifacts/routeaudit_universal.json")
    p.add_argument("--out", default="artifacts/routeaudit_attacks.jsonl")
    p.add_argument("--shift-out", default="artifacts/routeaudit_routing_shift.json")
    ...
    ui.step_header(3, "RouteAudit — universal suffix search", total=4)
    loaded = load_model(cfg)
    attack_run(loaded, cfg, args)
    ui.print_done("RouteAudit complete")
