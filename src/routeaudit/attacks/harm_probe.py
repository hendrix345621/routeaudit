...
Why probe routing features, not the residual stream:
  - **mHC-immune**: the gate input is C-dim regardless of the n-stream residual, so
    this works on Qwen3.6 / DeepSeek where an Arditi-style residual probe can't.
...
def boundary_routing_features(router_logits: dict[int, torch.Tensor], boundary: int) -> torch.Tensor:
    ...
    independent of the residual stream (so mHC-safe)."""
    feats = []
